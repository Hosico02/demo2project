...
describe('CLI information architecture', () => {
  ...
  it('every Phase 8 command is wired in the switch', async () => {
    const indexTs = await fs.readFile(path.join(root, 'src', 'cli', 'index.ts'), 'utf8');
    for (const c of ['doctor', 'next', 'quickstart', 'config:show', 'config:validate', 'config:migrate', 'diagnose', 'report:project', 'report:html', 'claude:setup', 'github:install-workflows', 'extensions:list', 'recipes:list', 'compatibility', 'release:check', 'docs:check', 'product:score', 'ux:check', 'examples:list']) {
